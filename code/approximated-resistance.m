...
model_choice = "sick-upper";		#"rain" for the rain model, "sick" for the sickness model, "random" for autogeneration
...
function [g_b, t_m_a, s_m_a, u_v] = choose_model(s)	#model choice
	...
	case "sick-lower"				
		...
		t_m_a(1).val = [0.9, 0.1;
				0.4, 0.6];
		s_m_a(1).val = [0.2, 0.05, 0.05;
				0.1, 0.6, 0.3];
		u_v = [1, -2];
		
	case "sick-upper"				
		g_b = [0.67, 0.33];
		t_m_a(1).val = [0.95, 0.05;
				0.4, 0.6];
		s_m_a(1).val = [0.9, 0.7, 0.1;
				0.1, 0.6, 0.3];
		u_v = [0, -2];
...
function integrity_checks(o_v, g_b, t_m_a, s_m_a, u_v)						#integrity checks on the input parameters
	...
	for i=1:length(s_m_a)									#on the sensor model array
		...
		#		printf("ERROR: impossible %d-th sensor model, %d-th row\n", i, j);
		#		exit;
		#	endif
		#endfor
	endfor
